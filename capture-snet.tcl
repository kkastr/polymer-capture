...
proc fmd {} {
	global N tmax z_line trans_flag t_trans cx cy cz nmax filename rseed n_attempt part_pos_trans part_pos_contact force
	...
	while  {$flag == 0} {
		...
		set cr2 [expr $cx*$cx + $cy*$cy + $cz*$cz]
		set cr [expr sqrt($cr2)]
		#puts $rmin
		#puts $r_min
		if { $r_min > 30.0} {
		...
		if { $rmin < 1.0 && $rg_flag == 0 } {
			#puts "inside if statement for rmin to pore"
			set t_contact $t
			set rg_calc [analyze rg 0 1 $N]
			set radius_of_gyration [open "data/${filename}_$N/radius_of_gyration_$N-$rseed" "a"]
