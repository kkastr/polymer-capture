...
void calc_rg(double **_rg)
{
  ...
  double M;
  double phi,theta;
  double r_Gx=0.0,r_Gy=0.0,r_Gz=0.0;
  ...
  *_rg = rg = realloc(rg,4*sizeof(double));
  ...
  for (i=0; i<chain_n_chains; i++) {
    ...
    for (j=0; j<chain_length; j++) {
      ...
      r_CM_x += partCfg[p].r.p[0]*PMASS(partCfg[p]) - 250.0;
      r_CM_y += partCfg[p].r.p[1]*PMASS(partCfg[p]) - 250.0;
      r_CM_z += partCfg[p].r.p[2]*PMASS(partCfg[p]) - 250.0;
      M += PMASS(partCfg[p]);
    }
    r_CM_x /= M;
    r_CM_y /= M;
    ...
    theta = atan(-r_CM_z/r_CM_y);
    phi = atan(-(cos(theta)*r_CM_y - sin(theta)*r_CM_z)/r_CM_x);

    for (j=0; j<chain_length; ++j) {
      p = chain_start+i*chain_length + j;
      dx = partCfg[p].r.p[0] - 250.0;
      dy = partCfg[p].r.p[1] - 250.0;
      dz = partCfg[p].r.p[2] - 250.0;



      tx = dx*cos(phi) - dy*sin(phi)*cos(theta) + dz*sin(phi)*cos(theta);
      ty = dx*sin(phi) + dy*cos(phi)*cos(theta) - dz*cos(phi)*sin(theta);
      tz = dy*sin(theta) + dz*cos(theta);
